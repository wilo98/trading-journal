[index.html.html](https://github.com/user-attachments/files/25418643/index.html.html)
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
<div class="app">
  ...
  <div class="nav">
    <div class="nav-left">
      <div class="nav-title">📊 TRADING JOURNAL</div>
  ...
  <div class="page active" id="page-dashboard">
    ...
    <div class="kpi-strip">
      ...
      <div class="kpi" style="--kc:var(--cyan)">
        ...
        <div class="kpi-value" id="k-r">—</div>
      ...
      <div class="kpi" style="--kc:var(--yellow)">
        ...
        <div class="kpi-value" id="k-avgw">—</div>
        <div class="kpi-sub" id="k-avgl-lbl">R prom. perdedor: <span id="k-avgl">—</span></div>
      </div>
    </div>
    ...
    <div class="dash-grid">
      ...
      <div style="display:flex;flex-direction:column;gap:18px;">
        ...
        <!-- SESSION -->
        <div class="panel">
          <div class="panel-title">Win Rate · Sesión</div>
    ...
    <div class="dash-bottom">
      ...
      <!-- HEATMAP + PLAN -->
      <div class="panel">
        <div class="panel-title">Win Rate · Día Semana</div>
        ...
        <div class="bar-group">
          <div class="bar-row"><div class="bar-label">Siguió plan</div><div class="bar-track"><div class="bar-fill bf-green" id="plan-yes" style="width:0%"><span></span></div></div></div>
          <div class="bar-row"><div class="bar-label">Forzó entrada</div><div class="bar-track"><div class="bar-fill bf-red" id="plan-no" style="width:0%"><span></span></div></div></div>
        </div>
        <div class="panel-title" style="font-size:10px;margin-top:18px;margin-bottom:10px;"># Trades por día</div>
        <div class="num-trades" id="num-trades-bars"></div>
      </div>

      <!-- MONTHLY -->
      <div class="panel">
        <div class="panel-title">R Acumulado · Mensual</div>
        <div class="monthly-bars" id="monthly-bars"></div>
        <div class="panel-title" style="font-size:10px;margin-top:18px;margin-bottom:12px;">Expectancy · Mensual</div>
        <div class="bar-group" id="monthly-exp-bars"></div>
      </div>

      <!-- SETUP + EMO -->
      <div class="panel">
        <div class="panel-title">Win Rate · Setup</div>
        <div class="bar-group" id="setup-bars"></div>
        <div class="panel-title" style="font-size:10px;margin-top:18px;margin-bottom:10px;">Estado Emocional → Win Rate</div>
  ...
  <div class="page" id="page-trades">
    <div class="table-toolbar">
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <input class="search-box" type="text" placeholder="🔍 Buscar setup, activo…" id="searchInput" oninput="renderTable()">
      ...
      <div style="display:flex;gap:8px;">
        <button class="btn btn-danger" onclick="clearAllTrades()">🗑 Borrar todo</button>
...
function load() {
  try {
    ...
    const c = localStorage.getItem('tj_config');
...
function save() {
  ...
  localStorage.setItem('tj_config', JSON.stringify(config));
}
...
function saveConfig() {
  ...
  save(); closeConfig(); refreshAll(); toast('Configuración guardada ✓');
}

// ══════════════════════════════════════════════════════
//  HELPERS
// ══════════════════════════════════════════════════════
function fmt(n, decimals=2) { return isFinite(n) ? n.toFixed(decimals) : '—'; }
function fmtR(n) { if(!isFinite(n)) return '—'; return (n>=0?'+':'')+n.toFixed(2)+'R'; }
function fmtD(n) { if(!isFinite(n)) return '—'; return (n>=0?'+$':'−$')+Math.abs(n).toLocaleString('es',{minimumFractionDigits:2,maximumFractionDigits:2}); }
...
function calcStats() {
  ...
  // Plan vs forced
  const planYes = trades.filter(t=>t.plan==='Sí');
...
function refreshAll() {
  ...
  document.getElementById('nav-sub').textContent =
    `${config.name} · ${d.toLocaleDateString('es',{weekday:'long',day:'numeric',month:'long'})}`;
...
function updateKPIs(s) {
  ...
  document.getElementById('k-pf').textContent  = isFinite(s.pf)?fmt(s.pf):'∞';
...
function renderHeatmap(s) {
  ...
  days.forEach(d=>{
    ...
    el.innerHTML+=`<div class="heat-cell" style="background:${bg}">
      ...
      <div class="heat-val" style="color:${dd.total?color:'var(--muted)'}">${dd.total?(wr*100).toFixed(0)+'%':'—'}</div>
...
function renderEmo(s) {
  ...
  [1,2,3,4,5].forEach((e,i)=>{
    ...
    el.innerHTML+=`<div class="emo-row">
      ...
      <div class="emo-val">${d.total?(d.wr*100).toFixed(0)+'%':'—'}</div>
    </div>`;
  });
}
...
function updateTrade(id, field, value) {
  ...
  t[field]=value;
  save();
  // only full refresh on R or risk change
...
function renderTable() {
  ...
  const tb=document.getElementById('tradesBody');
  if(!filtered.length){
    tb.innerHTML=`<tr><td colspan="18"><div class="empty-state"><div class="es-icon">📋</div><p>No hay trades registrados.<br>Haz clic en <strong>+ Nuevo Trade</strong> para comenzar.</p></div></td></tr>`;
  ...
  tb.innerHTML=filtered.map((t,idx)=>{
    ...
    return `<tr data-id="${t._id}">
      ...
      <td class="${dClass}" style="font-size:9px">${rVal!==0?(parseFloat(resultD)>=0?'+$':'−$')+Math.abs(parseFloat(resultD)).toFixed(2):'—'}</td>
      ...
      <td><select class="emo-select" onchange="updateTrade('${t._id}','emo',this.value)">
        ${[['1','😞'],['2','😐'],['3','🙂'],['4','😊'],['5','🔥']].map(([v,e])=>`<option value="${v}"${t.emo===v?' selected':''}>${e} ${v}</option>`).join('')}
      </select></td>
      <td><select onchange="updateTrade('${t._id}','plan',this.value)">
        <option${t.plan==='Sí'?' selected':''}>Sí</option><option${t.plan==='No'?' selected':''}>No</option>
      </select></td>
      <td>${badge}</td>
      <td class="td-note"><input type="text" value="${t.error||''}" placeholder="sin error" onchange="updateTrade('${t._id}','error',this.value)"></td>
      <td class="td-url"><input type="text" value="${t.url||''}" placeholder="link…" onchange="updateTrade('${t._id}','url',this.value)"
        onclick="if(this.value)window.open(this.value,'_blank')"></td>
      <td><button class="del-btn" onclick="deleteTrade('${t._id}')">✕</button></td>
    </tr>`;
  }).join('');
...
function updateTableSummary() {
  ...
  el.innerHTML=`
    ...
    <div class="ts-item">Profit Factor: <span>${isFinite(s.pf)?fmt(s.pf):'∞'}</span></div>
  `;
}
...
function exportCSV() {
  ...
  const headers=['Fecha','Sesión','Activo','Tipo','Setup','Riesgo%','Riesgo$','RR Plan','Resultado R','Resultado $','Duración','Emo','Siguió Plan','Error','Screenshot'];
...
function clearAllTrades() {
  if(!confirm('¿Borrar TODOS los trades? Esta acción no se puede deshacer.')) return;
  trades=[];save();renderTable();refreshAll();toast('Trades eliminados');
}
